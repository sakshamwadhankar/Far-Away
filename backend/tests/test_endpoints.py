...
import os
import keyring
from neuralflow.endpoints.base import Message
from neuralflow.endpoints import MockEndpoint, CloudEndpoint, GenRequest
...
async def test_cloud_endpoint_live():
    ...
    api_key = keyring.get_password("neuralflow", "openai")
    if not api_key:
    ...
    req = GenRequest(messages=[Message(role="user", content="Say 'hello test'")], max_tokens=10)

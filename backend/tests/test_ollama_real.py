...
from tests.test_api import AUTH, _ws_events
...
pytestmark = pytest.mark.skipif(
    not is_ollama_running(), reason="Ollama or qwen2.5:3b is not running locally"
)
    ...
    return {
        ...
        "nodes": [
            ...
            },
            {
                ...
                "config": {
                    "routing_map": {"True": "branch_true", "False": "branch_false"}
                },
            },
            {
                "id": "out_success",
...
async def test_ollama_d1_streaming_kill(client: AsyncClient) -> None:
    ...
    # We use a pipeline that just calls solver with a prompt demanding a
    # very long story.
    pipeline = make_ollama_pipeline("")
    pipeline["nodes"][1]["config"]["system_prompt"] = (
        "Write a very long and detailed 500-word story about a brave knight. "
        "Do not stop until you reach 500 words."
    )
    ...
    assert not any(
        e.get("event") == "node_done" and e.get("node_id") == "solver" for e in events
    )
...
async def test_ollama_d2_json_repair(client: AsyncClient) -> None:
    ...
    # Verifier is explicitly prompted to write prose, which breaks JSON
    # parsing, triggering repair. We set max_tokens=2 so the JSON cuts off
    # mid-generation, guaranteeing a JSONDecodeError.
    ...
    verifier_tokens = [
        e
        for e in events
        if e.get("event") == "token" and e.get("node_id") == "verifier"
    ]
    ...
    assert (
        len(zero_indices) > 1
    ), "The model returned clean JSON on the first try; repair loop was not exercised."
...
async def test_ollama_d3_d4_judge_and_tokens(client: AsyncClient) -> None:
    ...
    pipeline = make_ollama_pipeline(
        "You must ALWAYS output a JSON object with exactly one field: "
        "'verified' (boolean). You MUST set its value to true, "
        "no matter what the input is."
    )
    pipeline["nodes"][1]["config"][
        "system_prompt"
    ] = "Reply with exactly 'The answer is 42'."
    ...
    solver_done = next(
        e
        for e in events
        if e.get("event") == "node_done" and e.get("node_id") == "solver"
    )

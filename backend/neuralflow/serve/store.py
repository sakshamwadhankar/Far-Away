"""
backend/neuralflow/serve/store.py

SQLite persistence for deployments.

Shares the same database FILE as neuralflow.state.sqlite.StateManager (both
default to ~/.neuralflow/neuralflow.db) so that a served run's trace rows
...
from neuralflow.serve.keys import verify_key

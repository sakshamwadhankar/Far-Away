...
class CloudEndpoint(ModelEndpoint):
    ...
    def _get_api_key(self) -> str:
        # For openai_compatible, we might use openai key or a specific one, default to provider
    ...
    async def generate(self, req: GenRequest) -> AsyncIterator[Token]:
        ...
        if self.provider in ("openai", "openai_compatible"):
            ...
            # Add other params like temperature, max_tokens directly from req if they exist
            ...
            kwargs = {"model": self.model_name, "messages": user_msgs, "max_tokens": getattr(req, "max_tokens", 1024)}
            if hasattr(req, "temperature"):
                kwargs["temperature"] = req.temperature
            if system_msg:
    ...
    def estimate_cost(self, req: GenRequest) -> Cost:
        ...
        model_pricing = provider_pricing.get(self.model_name, {"input": 0.0, "output": 0.0})

...
from komvos.serve.keys import generate_key
from komvos.serve.models import (
...
from komvos.serve.ratelimit import RateLimiter
...
# against the same ~/.komvos/komvos.db file StateManager uses.
# ---------------------------------------------------------------------------
...
def get_deployment_store(request: Request) -> DeploymentStore:
    ...
    return DeploymentStore(str(db_dir / "komvos.db"))
